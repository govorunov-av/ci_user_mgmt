...
delete_user_and_resources() {
    ...
    #Удаляем группы безопасности
    sleep 2
    for id in $(openstack security group list --project "$project_name" -c ID -f value $INSECURE); do
        openstack security group delete "$id" $INSECURE
        echo "Удалена группа безопасности: $id"
    done
